...
public class Enemy {
		...
		while (cont) {
			...
			if (currentD[0] ==2 || counter == 20) {
				...
				checkpoints.add(FindNextC(checkpoints.get(counter).getTile(), 
						directions = FindNextD(checkpoints.get(counter).getTile()))); //wat
			}
			counter++;
		}
	}
	
	private Checkpoint FindNextC(Tile s, int[] dir) { // start tile, direction
		Tile next = null;
		Checkpoint c = null;
		
		// Boolean to decide iff next checkpoint is found
		boolean found = false;
	...
	private int[] FindNextD(Tile s) { // find next direction, takes a start tile as argument
		int[] dir = new int[2];
		Tile u = grid.GetTile(s.getXPlace(), s.getYPlace() - 1); // tile above
		Tile r = grid.GetTile(s.getXPlace() + 1, s.getYPlace()); // tile right
		Tile d = grid.GetTile(s.getXPlace(), s.getYPlace() + 1); // tile below
		Tile l = grid.GetTile(s.getXPlace() + 1, s.getYPlace()); // tile left
